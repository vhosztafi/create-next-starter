...
export async function createProject(options: ProjectOptions) {
  ...
  try {
    ...
    await execa('git', ['commit', '-m', 'chore: bootstrap from create-next-starter'], { cwd: projectPath })
...
async function applyOverlay(projectPath: string, overlayName: string, templatesDir: string) {
...
async function findAllPartialFiles(dir: string): Promise<string[]> {
  ...
  async function search(currentDir: string) {
    ...
    for (const entry of entries) {
      ...
      } else if (entry.name === 'package.json.partial' || entry.name.match(/^package\..*\.partial$/)) {
        files.push(fullPath)
      }
    }
  }
...
async function findPartialFiles(dir: string, pattern: string): Promise<string[]> {
  const files: string[] = []
...
function printSuccessMessage(projectName: string, provider: string, storybook: boolean) {
